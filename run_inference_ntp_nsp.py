...
def main(**kwargs):
    ...
    os.makedirs(f"{train_config.dist_checkpoint_folder}", exist_ok=True)
    os.makedirs(f"../{train_config.dist_checkpoint_folder}", exist_ok=True)
    emb_path = f"../{train_config.dist_checkpoint_folder}/{dataset_config.dataset}_top{dataset_config.ndocs}.emb"
    if train_config.ret_first:
        temp_path = f"{train_config.dist_checkpoint_folder}/ret_first.{dataset_config.dataset}_top{dataset_config.ndocs}.tmp"
        save_path = f"{train_config.dist_checkpoint_folder}/ret_first.{dataset_config.dataset}_top{dataset_config.ndocs}"
    else:
        temp_path = f"{train_config.dist_checkpoint_folder}/{dataset_config.dataset}_top{dataset_config.ndocs}.tmp"
        save_path = f"{train_config.dist_checkpoint_folder}/{dataset_config.dataset}_top{dataset_config.ndocs}"
    ...
    if os.path.exists(temp_path):
        ...
        if os.path.exists(emb_path):
            ...
            dataloader = torch.utils.data.DataLoader(
                ctx_dataset,
                batch_size = 10, # train_config.val_batch_size,
            ...
            gathered_ctx = dump(model, train_config, dataloader, None, world_size)
            with open(emb_path, "wb") as f:
                pickle.dump(gathered_ctx, f)
            accelerator.wait_for_everyone()
            print("Done dumping embeddings!")
        ...
        print(f"*** ctx_truncate: {dataset_config.ctx_truncate}")
        gathered_results, gathered_ret_idx = test_alce(
            model,
            train_config,
            dataset_config,
            dataloader,
        ...
        if not os.path.exists(temp_path) and os.path.exists(save_path):
            ...
            if "data" in save_data:
                if len(save_data['data']) == len(input_data):
                    print(f"Done!")
                    os.system(f"python eval_metric.py --f {save_path} --data_name {dataset_config.dataset}")
                    sys.exit(-1)
        save_data = {}
        ...
        for instance_idx in tqdm(range(len(input_data))):
            ...
            for doc_item in ctxs:
                if doc_item["id"] in inf_ret_idx:
                    ...
                    retIdx2doc[doc_item["id"]] = doc_item
            item["docs"] = doc_items
            ...
            for doc_text in docs:
                ...
                for doc_item in ctxs:
                    if doc_item["text"] in doc_text:
                        ...
                        break
                if flag ==False:
                    # double check if the issue is not because of tokenization
                    for doc_item in ctxs:
                        text = doc_item["text"]
                        text = eval_tokenizer.decode(eval_tokenizer(text)["input_ids"])
                        if text in doc_text:
                            flag = True 
                            doc_items.append(doc_item)
                            break
...
def prepare(train_config,dataset_config, model_ckpt_path, type, kwargs):
    # Load the tokenizer and add special tokens
    eval_tokenizer = LlamaTokenizer.from_pretrained(
        train_config.model_name, model_max_length=1024
    )
    ...
    model_ckpt_path = os.path.join(model_ckpt_path, type)
    if type == "ctx":
    ...
    # do_print(f"Model embed token size => {model.get_input_embeddings().weight.shape}")
